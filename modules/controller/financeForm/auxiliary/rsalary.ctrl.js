angular.module('core').controller('rsalaryCtrl', ['$scope', '$http','$rootScope','commonService','$timeout','$compile','$state',"$stateParams","$window",
    function ($scope, $http,$rootScope,commonService,$timeout,$compile,$state,$stateParams,$window) {
        ...
        $(".rsalary").height(height-250);

        commonService.getRSalary($scope.schemeId).then((respon)=>{
            ...
            commonService.getPeriod($scope.schemeId).then((res)=>{
                ...
                for(let i=0;i<count;i++){
                    ...
                    let temp = $compile(html)($scope);
                     angular.element($(".rsalary .tableAuto thead tr:last-child")).append(temp);
                }
