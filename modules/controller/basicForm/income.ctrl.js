angular.module("core").controller('incomeCtrl', ['$scope', '$http','commonService','$uibModal','$window','$compile','$interval',
    function ($scope, $http, commonService,$uibModal,$window,$compile,$interval) {
        ...
        window.incomeChange=false;
        function getChange(){
            $(".income .body tbody tr input").change(function() {
                window.incomeChange=true;
            });
        }
        ...
        commonService.findTaxSys(parseFloat(tId),$scope.schemeId).then((res)=>{
            ...
            commonService.getIncome($scope.schemeId).then((res)=> {
                if(res.result.length==0||res.success==false){
                    return
                }
                let list = res.result;
                list.map((item,index)=>{
                    let amount=item.amount==null?"":item.amount;
                    let outPutUnit=item.outPutUnit==null?"":item.outPutUnit;
                    ...
                    let ntaxPrice=item.ntaxPrice==null?"":item.ntaxPrice;
                    let ntaxPrices=item.ntaxPrice==null?"":item.ntaxPrice.toFixed(2);
                    let businessIncome=item.businessIncome==null?"":item.businessIncome;
                    let businessIncomes=item.businessIncome==null?"":item.businessIncome.toFixed(2);
                    let html = `<tr ng-click="click($event)" data-id=${item.id}>
                    <td width="8%">${index+1}</td>
                    <td width="20%"><input type="text" maxlength="20"  set-focus value=${item.itemName==null?"":item.itemName}></td>
                    <td width="12%" class="amount"><input type="number" min="1" ng-keyup="aduitTax($event,'数量')" set-focus value="${amount}"></td>
                    <td width="12%" class="selTd">
                        <input type="text" maxlength="20" onkeyup="this.value=this.value.replace(/[^\a-zA-Z\u4E00-\u9FA5]/g,'')" value="${outPutUnit}" set-focus>
                    </td>
                    <td width="12%" class="selTd">
                    ...
                    <td width="12%"><input type="number" min="0" data-num="${businessIncome}" readonly="readonly" set-focus value="${businessIncomes}"></td>
                </tr>`;
                    let temp = $compile(html)($scope);
                    angular.element($(".income .body tbody .total")).before(temp);
                })
                let tr = $(".income .body table tr");
                let length = tr.length;
                $(".income .total>td:nth-child(1)").text(length);
                setHeigth();
                setTotal();
                setTaxType();
                getChange();
        ...
        $scope.add = ()=>{
            $(".income .body tr").removeClass("add");
            let tr = $(".income .body table tr");
            let length = tr.length;
            let html = `<tr class="add" ng-click="click($event)">
                ...
                <td width="12%" class="selTd">
                    <input type="text" maxlength="20" onkeyup="this.value=this.value.replace(/[^\a-zA-Z\u4E00-\u9FA5]/g,'')" set-focus>
                </td>
                <td width="12%" class="selTd">
            ...
            let temp = $compile(html)($scope);
            angular.element($(".income .total")).before(temp);
            $(".income .total>td:nth-child(1)").text(length+1);
            commonService.income_addRow($scope.projectId,$scope.schemeId).then((res)=>{
                if(res.success==true){
                    $(".income .body .add").attr("data-id",res.result);
                }
            })
            setHeigth();
            setTaxType();
        ...
        $(".income").height(height*0.76-130);
        $(".income .tableCon").height(height*0.76-320);
        $(".income .tableCon .body").height(height*0.76-350);
        function setHeigth(){
            if($(".income .body table").height()>$(".income .body").height()){
                $(".income .head").css({"paddingRight":"26px"});
            }else{
                $(".income .head").css({"paddingRight":"20px"});
            }
        }
        $scope.click = (e)=>{
            let _this = $(e.target);
            if(_this.parent("tr").hasClass("total")){
                return;
            }else{
                $(".income .body table tr").removeClass("inputClick");
        ...
        $scope.delete = ()=>{
            ...
            if(tr.hasClass("inputClick")){
                let id = parseInt($(".income .body table .inputClick").attr("data-id"));
                commonService.income_delRow(id).then((res)=>{
                    if(res.success==true){
                        $(".income .body table .inputClick").remove();
                        tr.map((index,value)=>{
        ...
        function setTaxType(){
            if($scope.taxType==0){//含税
                $(".income .body tr:not(.total) td:nth-child(6) input").removeAttr("readonly").removeClass("readonlyStyle");
                $(".income .body tr:not(.total) td:nth-child(7) input").attr("readonly","readonly").addClass("readonlyStyle");
            }else{
                $(".income .body tr:not(.total) td:nth-child(7) input").removeAttr("readonly").removeClass("readonlyStyle");
                $(".income .body tr:not(.total) td:nth-child(6) input").attr("readonly","readonly").addClass("readonlyStyle");
            }
        }
        //价格体系、单位
        $scope.selectTax = (num)=>{
        ...
        $scope.selectUnit = (num)=>{
            if($scope.unitType!=num){
                //万元更改为元
                if($scope.unitType==0){
                    changeUnitType($(".income .body tr:not(.total) td:nth-child(6) input"),"*");
                    changeUnitType($(".income .body tr:not(.total) td:nth-child(7) input"),"*");
                }else{
                    //元更改为万元
                    changeUnitType($(".income .body tr:not(.total) td:nth-child(6) input"),"/");
                    changeUnitType($(".income .body tr:not(.total) td:nth-child(7) input"),"/");
        ...
        $scope.aduitTax = ($event,str)=>{
            ...
            if($scope.taxType == 1){
                if(addTax!=undefined){
                    //含税单价
                    let value = parseFloat(nTax)*(1+parseFloat(addTax)/100);
                    _this.parents("tr").find("td:nth-child(6) input").attr("data-num",value);
                    _this.parents("tr").find("td:nth-child(6) input").val(value.toFixed(2));
                    if(count!=""){
                        let values;
                ...
                if(addTax!=undefined){
                    //不含税单价
                    let value = parseFloat(hTax)/(1+parseFloat(addTax)/100);
                    _this.parents("tr").find("td:nth-child(7) input").attr("data-num",value);
                    _this.parents("tr").find("td:nth-child(7) input").val(value.toFixed(2));
                    if(count!=""){
                        let values;
        ...
        function setTotal(){
            let values=0;
            $(".income .body table tr:not(.total) td:last-child").map((index,value)=>{
                let val = $(value).find("input").attr("data-num");
                val = (isNaN(val)||val==undefined)?0 : parseFloat(val);
                values+=parseFloat(val);
            })
            $(".income .body table tr:last-child td:last-child").attr("data-num",values);
            $(".income .body table tr:last-child td:last-child").text(values.toFixed(2));
        }
